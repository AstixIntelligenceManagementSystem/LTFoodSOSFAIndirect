						...
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog, int which)
							{
								...
								dbengine.saveInvoiceButtonStoreTransac("NA",TagDate,TagStoreID,"0","0",0.0, 0,
										0, 0,TagOrderID,"","9",1,0.0,TagRouteID,"0",et_Reason.getText().toString().trim());
								dbengine.close();
					...
					//	dialog.dismiss();
						...
						for (int countRow = 0; countRow <= (ProductID.length - 1); countRow++)
						{
							...
							dbengine.saveInvoiceButtonStoreTransac(imei,TagDate,
									...
									DelQty[countRow], fQty[countRow],TagOrderID,CurrentRowCategoryId,"9",0,dVal[countRow],TagRouteID,additionalDiscountValue,et_Reason.getText().toString().trim());
